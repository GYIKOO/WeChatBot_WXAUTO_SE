            ...
            with queue_lock: # 确保对 chat_contexts 的访问是线程安全的
                ...
                if cot_prefill:
                    messages_to_send.append({"role": "system", "content": (
                        ...
                        "思维链做五件事：①情境判断，②反应推演，③长度与节奏，④合理性自检，⑤雷区预判。"
...
def summarize_and_save(user_id, skip_check=False):
    ...
    try:
        ...
        # --- 生成总结 ---
        summary_prompt = (
            f"请以{prompt_name}的视角，用中文将以下与{user_id}的对话总结成一段记忆片段。\n"
            f"要求：\n"
            f"1. 只输出一个完整的自然段，不分段、不分条\n"
            f"2. 全部用自己的话转述，禁止引用或复制原文对话、括号动作描写\n"
            f"3. 概括发生了哪些事、大致聊了什么，保留关键事件和重要细节\n"
            f"4. 按时间顺序叙述，不评价性格、情感或关系\n"
            f"5. 严格控制在150字以内\n"
            f"6. 直接输出正文，不加任何标题或前缀\n\n"
            f"{full_dialogue}"
        )
