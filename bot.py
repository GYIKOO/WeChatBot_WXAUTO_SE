...
def summarize_and_save(user_id, skip_check=False):
    ...
    try:
        ...
        # --- 生成总结 ---
        summary_prompt = (
            f"请以{prompt_name}的视角，用中文将以下与{user_id}的对话整理成一段记忆片段。\n"
            f"要求：\n"
            f"1. 只输出一个完整的自然段，不分段、不分条、不加括号\n"
            f"2. 记录具体发生了什么事、说了什么话，保留关键细节（如具体行为、物品、原因、对方说的话）\n"
            f"3. 按时间顺序叙述，不评价性格、情感或关系\n"
            f"4. 严格控制在200字以内，超出时优先保留最重要的事件\n"
            f"5. 直接输出正文，不加任何标题或前缀\n\n"
            f"{full_dialogue}"
        )
